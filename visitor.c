...
AST_T* visitor_visit_statement_definition(visitor_T* visitor, AST_T* node)
{
    ...
    else if (strcmp(node->statement_definition_type, "while") == 0)
    {
        while (visitor_visit_boolean(visitor, node->statement_definition_args[0])->boolean_value == 1)
        {
            ...
            node->statement_definition_body->scope = init_scope();
        }
    }
...
AST_T* visitor_visit_function_call(visitor_T* visitor, AST_T* node)
{
    ...
    if (strcmp(node->function_call_name, "println") == 0)
    {
        for (int i = 0; i < args_size; i++)
        {
            ...
            switch (visited_ast->type)
            {
                ...
                case AST_INT: printf("%i\n", visited_ast->ast_int); break;
                case AST_DOUBLE: printf("%f\n", visited_ast->ast_double); break;
                case AST_BOOLEAN:
                    switch (visited_ast->boolean_value)
                    {
                        case 1: printf("true\n"); break;
    ...
    if (strcmp(node->function_call_name, "atoi") == 0)
    {
        ...
        visited_ast->ast_int = atoi(visitor_visit(visitor, args[0])->string_value);
        return visited_ast;
    }
    ...
    {
        AST_T* visited_ast = visitor_visit(visitor, args[0]);
        visited_ast->ast_int = visited_ast->array_size;
        return visited_ast;
    }
...
AST_T* visitor_visit_math_expr(visitor_T* visitor, AST_T* node)
{
    ...
    {
        case AST_INT: ast->ast_double = ast_value->ast_int; break;
        case AST_DOUBLE: ast->ast_double = ast_value->ast_double; break;
    }
    for (int i = 1; i < node->math_expression_size; i++)
    {
        ...
        {
            case '+': ast->ast_double += value; break;
            case '-': ast->ast_double -= value; break;
            case '/': ast->ast_double /= value; break;
            case '*': ast->ast_double *= value; break;
        }
